...
class Core:
    ...
    def get_result(self, model, time_diff=False, td_transform=None):
        """
        Docstring
        """
        # Frequancy Domain
        ...
        # Time Domain
            ...
            # Adaptive Convolution
                ...
                nb = np.int(np.floor(10 * np.log(self.freqtime[-1] / self.freqtime[0])) + 1)
                ...
                if not time_diff:
                    ...
                    for ii, emfield in enumerate(emfield):
                        time_ans, arg = transform.FourierTransform.dlagf1em(model, nb, emfield)
                        ans[:, ii] = time_ans / arg
