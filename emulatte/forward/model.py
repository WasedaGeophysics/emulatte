...
class Subsurface1D:
    ...
    def emulate(self, hankel_filter, time_diff=False, td_transform=None):
        ...
        ans = self.tcv.get_result(self, time_diff=time_diff, td_transform=td_transform)
    ...
    def compute_coefficients(self, omega):
        ...
        # 誘電率を無視する近似
        k[0] = 0
        k[1:self.num_layer] = (- 1j * omega * self.mu[1:self.num_layer] * self.sigma) ** 0.5
        ...
        for ii in range(self.num_layer - 1, ti, -1):  # (2) Ytilde{self.num_layer-1,self.num_layer-2,\,ti}
        ...
        for ii in range(2, ti):
            ...
            Yhat[ii - 1] = Y[ii - 1] * numerator_Y / denominator_Y  # (2)Yhat{2,3,\,ti-2,ti-1}
        ...
        if ti == 1:
            U_te[ti - 1] = 0
            U_tm[ti - 1] = 0
            D_te[ti - 1] = self.kernel_te_down_sign * r_te[ti - 1] * np.exp(-u[ti - 1] * (self.depth[ti - 1] - self.tz))
            D_tm[ti - 1] = self.kernel_tm_down_sign * r_tm[ti - 1] * np.exp(-u[ti - 1] * (self.depth[ti - 1] - self.tz))
        elif ti == self.num_layer:
            U_te[ti - 1] = self.kernel_te_up_sign * R_te[ti - 1] * np.exp(u[ti - 1] * (self.depth[ti - 2] - self.tz))
            U_tm[ti - 1] = self.kernel_tm_up_sign * R_tm[ti - 1] * np.exp(u[ti - 1] * (self.depth[ti - 2] - self.tz))
            D_te[ti - 1] = 0
            D_tm[ti - 1] = 0
        else:
            exp_term1 = np.exp(-2 * u[ti - 1] * (self.depth[ti - 1] - self.depth[ti - 2]))
            exp_term2u = np.exp( u[ti - 1] * (self.depth[ti - 2] - 2 * self.depth[ti - 1] + self.tz))
            exp_term2d = np.exp(-u[ti - 1] * (self.depth[ti - 1] - 2 * self.depth[ti - 2] + self.tz))
            exp_term3u = np.exp( u[ti - 1] * (self.depth[ti - 2] - self.tz))
            exp_term3d = np.exp(-u[ti - 1] * (self.depth[ti - 1] - self.tz))

            U_te[ti - 1] = 1 / R_te[ti - 1] * (1 - R_te[ti - 1] * r_te[ti - 1] * exp_term1) * \
                            (self.kernel_te_down_sign * r_te[ti - 1] * exp_term2u + \
                             self.kernel_te_up_sign * exp_term3u)

            U_tm[ti - 1] = 1 / R_tm[ti - 1] * (1 - R_tm[ti - 1] * r_tm[ti - 1] * exp_term1) * \
                            (self.kernel_tm_down_sign * r_tm[ti - 1] * exp_term2u + \
                             self.kernel_tm_up_sign * exp_term3u)

            D_te[ti - 1] = 1 / r_te[ti - 1] * (1 - R_te[ti - 1] * r_te[ti - 1] * exp_term1) * \
                            (self.kernel_te_up_sign * R_te[ti - 1] * exp_term2d + \
                             self.kernel_te_down_sign * exp_term3d)

            D_tm[ti - 1] = 1 / r_tm[ti - 1] * (1 - R_tm[ti - 1] * r_tm[ti - 1] * exp_term1) * \
                            (self.kernel_tm_up_sign * R_tm[ti - 1] * exp_term2d + \
        ...
        if ri < ti:
            ...
            if ti == self.num_layer:
                D_te[ti - 2] = (Y[ti - 2] * (1 + R_te[ti - 1]) + Y[ti - 1] * (1 - R_te[ti - 1])) \
                                / (2 * Y[ti - 2]) * self.kernel_te_up_sign * exp_termi
                D_tm[ti - 2] = (Z[ti - 2] * (1 + R_tm[ti - 1]) + Z[ti - 1] * (1 - R_tm[ti - 1])) \
                                / (2 * Z[ti - 2]) * self.kernel_tm_up_sign * exp_termi

            elif ti != 1 and ti != self.num_layer:
                D_te[ti - 2] = (Y[ti - 2] * (1 + R_te[ti - 1]) + Y[ti - 1] * (1 - R_te[ti - 1])) \
                                / (2 * Y[ti - 2]) * (D_te[ti - 1] * exp_termii + self.kernel_te_up_sign * exp_termi)
                D_tm[ti - 2] = (Z[ti - 2] * (1 + R_tm[ti - 1]) + Z[ti - 1] * (1 - R_tm[ti - 1])) \
                                / (2 * Z[ti - 2]) * (D_tm[ti - 1] * exp_termii + self.kernel_tm_up_sign * exp_termi)
        ...
        if ri > ti:
            if ti == 1:
                ...
                U_te[ti] = (Y[ti] * (1 + r_te[ti - 1]) + Y[ti - 1] * (1 - r_te[ti - 1])) \
                            / (2 * Y[ti]) * self.kernel_te_down_sign * exp_term
                U_tm[ti] = (Z[ti] * (1 + r_tm[ti - 1]) + Z[ti - 1] * (1 - r_tm[ti - 1])) \
                            / (2 * Z[ti]) * self.kernel_tm_down_sign * exp_term

            elif ti != 1 and ti != self.num_layer:
                exp_termi = np.exp(-u[ti - 1] * (self.depth[ti - 1] - self.depth[ti - 2]))
                exp_termii = np.exp(-u[ti - 1] * (self.depth[ti - 1] - self.tz))
                U_te[ti] = (Y[ti] * (1 + r_te[ti - 1]) + Y[ti - 1] * (1 - r_te[ti - 1])) \
                            / (2 * Y[ti]) * (U_te[ti - 1] * exp_termi + self.kernel_te_down_sign * exp_termii)
                U_tm[ti] = (Z[ti] * (1 + r_tm[ti - 1]) + Z[ti - 1] * (1 - r_tm[ti - 1])) \
                            / (2 * Z[ti]) * (U_tm[ti - 1] * exp_termi + self.kernel_tm_down_sign * exp_termii)
