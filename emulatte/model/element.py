import numpy as np
def reflection_old(si, K, L, M, u, thick, admy, admz):
...
def tetm_mode_damping_old(model):
...
# for multi dipole
def reflection(si, J, K, L, M, u, thick, admy, admz):
    # yuc := yuc, zuc := zuc (uc : uppercase)
    tensor_thick = np.zeros((J, K, M, L), dtype=complex)
    tensor_thick[:,:,:,1:-1] = thick[1:-1]
    tensor_thick = tensor_thick.transpose((0,1,3,2))
    tanhuh = np.tanh(u * tensor_thick)
    tensor_admy = np.zeros((J, M, L, K), dtype=complex)
    tensor_admz = np.zeros((J, M, L, K), dtype=complex)
    tensor_admy[:,:] = admy.T
    tensor_admz[:,:] = admz.T
    tensor_admy = tensor_admy.transpose((0,3,2,1))
    tensor_admz = tensor_admz.transpose((0,3,2,1))
    yuc = u / tensor_admz
    zuc = u / tensor_admy
    
    yuc_down = np.ones((J, K, L, M), dtype=complex)
    zuc_down = np.ones((J, K, L, M), dtype=complex)
    # rlc := r, ruc := R (lc : lowercase)
    dr_te = np.ones((J, K, L, M), dtype=complex)
    dr_tm = np.ones((J, K, L, M), dtype=complex)
    ur_te = np.ones((J, K, L, M), dtype=complex)
    ur_tm = np.ones((J, K, L, M), dtype=complex)
    ...
    ### UP ADMITTANCE & IMPEDANCE ###
    yuc_up = np.ones((J, K, L, M), dtype=complex)
    zuc_up = np.ones((J, K, L, M), dtype=complex)
...
def tetm_mode_damping(model):
    ...
    dr_te, dr_tm, ur_te, ur_tm, yuc, zuc= reflection(
        si, J, K, L, M, u, thick, admy, admz
        )

    u_te = np.ones((J, K, L, M), dtype=complex)
    u_tm = np.ones((J, K, L, M), dtype=complex)
    d_te = np.ones((J, K, L, M), dtype=complex)
    d_tm = np.ones((J, K, L, M), dtype=complex)
    ...
    # compute Damping coefficient
    if ri == 0:
        e_up = np.zeros((J, K, M), dtype=complex)
        e_down = np.exp(u[:,:, ri] * (rz - depth[ri]))
    elif ri == L-1:
        e_up = np.exp(-u[:,:, ri] * (rz - depth[ri-1]))
        e_down = np.zeros((J, K, M), dtype=complex)
    else:
        e_up = np.exp(-u[:,:, ri] * (rz - depth[ri-1]))
        e_down = np.exp(u[:,:, ri] * (rz - depth[ri]))
